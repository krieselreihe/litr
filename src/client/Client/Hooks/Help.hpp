...
class Help {
 ...
 private:
  ...
  void print_command(const std::shared_ptr<config::Command>& command, const std::string& parent_name, size_t depth = 1) const;
  ...
  [[nodiscard]] static std::string get_command_name(const std::shared_ptr<cli::Instruction>& instruction);
  ...
  [[nodiscard]] static bool sort_parameter_by_required(const std::shared_ptr<config::Parameter>& p1, const std::shared_ptr<config::Parameter>& p2);
